...
rule run_all:
    input:
        expand('{}/{}'.format(config['path']['data'], 'bgs_1class/bgs_1class.B{B}_N{N}_rep{rep}.trees'), rep = reps, B = Bs, N = Ns)
...
rule simulate_BGS_validation:
    ...
    '''
    input:
        '{}/{}'.format(config['path']['script'], 'slim/bgs_1class.slim')
    output:
        '{}/{}'.format(config['path']['data'], 'bgs_1class/bgs_1class.B{B}_N{N}_rep{rep}.trees')
...
rule simulate_constant_size_vary_N:
    ...
    '''
    input:
        '{}/{}'.format(config['path']['script'], 'slim/bgs_3classes_standard.slim')
    output:
        '{}/{}'.format(config['path']['data'], 'bgs_3classes/bgs_3classes_N{N}_rep{rep}.trees')
    run:
...
rule simulate_bottleneck:
    ...
    '''
    input:
        '{}/{}'.format(config['path']['script'], 'slim/bgs_3classes_bottleneck.slim')
    output:
        '{}/{}'.format(config['path']['data'], 'slim/bgs_3classes_noneq/bgs_bottleneck_3classes.N500rep{rep}.trees')
    run:
...
rule simulate_bottleneck_no_mutation:
    ...
    input:
        '{}/{}'.format(config['path']['script'], 'slim/bgs_3classes_bottleneck.slim')
    output:
        '{}/{}'.format(config['path']['data'], 'slim/bgs_3classes_noneq/bgs_bottleneck_noMutation.N500rep{rep}.trees')
    run:
...
rule dadi_cache:
    ...
    '''
    input:
        '{}/{}'.format(config['path']['script'], 'spectrum_cache.py')
    output:
        '{}/{}'.format(config['path']['output'], '{cache_demo}_spectra_n{ns}.bpkl')
    run:
